...
radon mi cleaning_utils -s >>.logs/maintainability.txt
export maintainability=$(radon mi cleaning_utils -s | grep -Po '(?<=\().*(?=\))' | perl -lane '$a+=$_ for(@F);$f+=scalar(@F);END{print $a/$f}' | xargs printf "%.1f%%\n")
